          ...
          {focusAreas.map((area, index) => {
            ...
            return (
              <Reveal key={area.title} delay={(index % 3) * 0.08}>
                <Link
                  href={area.href}
                  className="group block h-full rounded-xl transition-transform focus-visible:ring-2 focus-visible:ring-brand-light focus-visible:ring-offset-2 focus-visible:outline-none"
                >
                  <Card className="h-full gap-0 p-6 transition-all duration-300 group-hover:-translate-y-1 group-hover:shadow-lg group-hover:ring-foreground/20">
                    <span className="mb-5 inline-flex size-12 items-center justify-center rounded-xl bg-brand-light/10 text-brand-light transition-colors group-hover:bg-brand-light group-hover:text-white">
